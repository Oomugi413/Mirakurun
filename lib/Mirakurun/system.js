...
function isPermittedIPAddress(addr) {
    const [isIPv4] = ip_num_1.Validator.isValidIPv4String(addr);
    if (isIPv4) {
        const ipv4 = new ip_num_1.IPv4CidrRange(new ip_num_1.IPv4(addr), new ip_num_1.IPv4Prefix(32n));
        for (const rangeString of _1.default.config.server.allowIPv4CidrRanges) {
            if (ipv4.inside(ip_num_1.IPv4CidrRange.fromCidr(rangeString))) {
                return true;
            }
        }
    }
    const [isIPv6] = ip_num_1.Validator.isValidIPv6String(addr);
    if (isIPv6) {
        const ipv6 = new ip_num_1.IPv6CidrRange(new ip_num_1.IPv6(addr), new ip_num_1.IPv6Prefix(128n));
        for (const rangeString of _1.default.config.server.allowIPv6CidrRanges) {
            if (ipv6.inside(ip_num_1.IPv6CidrRange.fromCidr(rangeString))) {
                return true;
            }
        }
    }
