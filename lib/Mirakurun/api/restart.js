...
const winService_1 = require("../winService");
...
const put = (req, res) => {
    if (process.env.pm_uptime) {
        ...
        const serviceName = (0, winService_1.getWindowsServiceName)(process.env);
        const cmd = (0, child_process_1.spawn)("cmd", ["/c", `ping -n ${SERVICE_START_WAIT_SEC} 127.0.0.1 > nul & sc start "${serviceName}"`], {
            detached: true,
            stdio: "ignore",
            windowsHide: true
        });
