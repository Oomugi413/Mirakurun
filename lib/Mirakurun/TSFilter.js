...
class TSFilter extends eventemitter3_1.EventEmitter {
    streamInfo = {};
    _output;
    _provideServiceId;
    _provideEventId;
    ...
    _onCDT(pid, data) {
        if (data.data_type === 0x01) {
            ...
            for (const service of _1.default.service.findByNetworkId(data.original_network_id)) {
                Service_1.default.saveLogoData(data.original_network_id, service.serviceId, dataModule.logo_id, logoData);
        ...
        if (data.table_id === 0x3C) {
            ...
            for (const logo of dataModule.logos) {
                for (const logoService of logo.services) {
                    ...
                    Service_1.default.saveLogoData(service.networkId, service.serviceId, service.logoId, logoData);
                    break;
                }
            }
        }
    ...
    async _standbyLogoData() {
        ...
        for (const networkId in logoIdNetworkMap) {
            for (const logoId of logoIdNetworkMap[networkId]) {
                ...
                if (now - await Service_1.default.getLogoDataMTime(this._targetNetworkId, this._provideServiceId, logoId) > logoDataInterval) {
                    if (this._closed) {
                        return;
                    }
